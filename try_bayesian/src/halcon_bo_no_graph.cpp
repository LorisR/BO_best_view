...
try_bayesian::to_bayesian msg_to_bayesian;
std::string  dir_dati_salv_bo,tempo,nome_file_completo;
std::string nome_file_halcon_bo = "halcon_bo_";
std::string name_file ="/home/loris/ply_and_stl/param.txt";;
std::string estensione = ".txt";
int only_one = 0;
int jj = 0;
float raggiungibilita;
...
void action()
{ ros::NodeHandle m;
  ros::Publisher bayesian_pub = m.advertise<try_bayesian::to_bayesian>("halcon_to_bayesian", 1000);
  ...
  //SampleObjectModel3d(hv_Scan,"accurate",0.01,HTuple(),HTuple(),&hv_Scan);
  ...
  //Lettura Viste e Creazione Modelli
  ...
  msg_to_bayesian.score = std::stof(score) ;
  msg_to_bayesian.check = raggiungibilita;
  bayesian_pub.publish(msg_to_bayesian);
...
{ 
  ...
  raggiungibilita = msg_da_pubb->check;
  dir_dati_salv_bo = readline(name_file,38,"{","}");
  if(only_one == 0){
    tempo = GetCurrentTimeForFileName();
    only_one =1;
  }

  nome_file_completo = dir_dati_salv_bo+nome_file_halcon_bo+tempo+estensione;
  std::cout << "\n\n ------------------------------------------";
